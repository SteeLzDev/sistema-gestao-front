...
export default function DetalhesVendaPage() {
  ...
  useEffect(() => {
    ...
    const carregarVenda = async () => {
      ...
      setError(null)
      try {
        console.log("Carregando venda com ID:", id) // Log para depuração
        const data = await vendaService.obterVenda(Number.parseInt(id))
        console.log("Dados da venda:", data) // Log para depuração
        setVenda(data)
      } catch (err) {
        console.error(`Erro ao carregar venda ${id}:`, err)
        setError("Erro ao carregar detalhes da venda. Tente novamente.")
      } finally {
        setLoading(false)
      }
    }
